...
const { useAssetsQueryMock, deleteManyMutateMock } = vi.hoisted(() => ({
...
vi.mock('@entities/asset', async () => {
  ...
  return {
    ...
    useDeleteManyAssetsMutation: () => ({ mutate: deleteManyMutateMock, isPending: false }),
  }
})
...
describe('AssetRecordList', () => {
  ...
  it('행을 선택하면 선택 삭제 바가 나타나고, 확인 시 선택 id로 삭제 mutation을 호출한다', async () => {
    ...
    useAssetsQueryMock.mockReturnValue({
      data: [asset({ id: 'a1' }), asset({ id: 'a2', subcategory: '정기예금' })],
      isLoading: false,
      isError: false,
    })
    render(<AssetRecordList />)
  ...
  it('행 단위 삭제 버튼은 해당 id 하나만으로 삭제 mutation을 호출한다', async () => {
    const user = userEvent.setup()
    useAssetsQueryMock.mockReturnValue({ data: [asset({ id: 'a1' })], isLoading: false, isError: false })
    render(<AssetRecordList />)
  ...
  it('기준일 헤더를 클릭하면 정렬 방향이 토글된다', async () => {
    ...
    useAssetsQueryMock.mockReturnValue({
      data: [
        asset({ id: 'a-old', entryDate: '2026-01-01', subcategory: '오래된기록' }),
        asset({ id: 'a-new', entryDate: '2026-08-01', subcategory: '최신기록' }),
      ],
      isLoading: false,
      isError: false,
    })
