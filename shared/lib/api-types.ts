/**
 * This file was auto-generated by openapi-typescript.
 * Do not make direct changes to the file.
 */

export interface paths {
    "/api/trading-cycles/{id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /** 거래 사이클 수정 */
        put: operations["update"];
        post?: never;
        /** 거래 사이클 삭제 */
        delete: operations["delete"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/settings/telegram": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 텔레그램 설정 조회
         * @description 현재 설정된 텔레그램 채팅 ID 반환. botToken은 보안상 응답에서 제외.
         */
        get: operations["getTelegram"];
        /**
         * 텔레그램 설정 저장
         * @description 텔레그램 봇 토큰과 채팅 ID를 AES-256 암호화하여 저장. body: {"botToken": "...", "chatId": "..."}
         */
        put: operations["updateTelegram"];
        post?: never;
        /**
         * 텔레그램 설정 해제
         * @description 저장된 텔레그램 봇 토큰과 채팅 ID를 삭제.
         */
        delete: operations["removeTelegram"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        /**
         * 계좌 수정
         * @description 별명을 수정. 계좌번호 및 KIS 자격증명은 수정 불가.
         */
        put: operations["update_1"];
        post?: never;
        /**
         * 계좌 삭제
         * @description 계좌 및 관련 데이터를 영구 삭제.
         */
        delete: operations["delete_1"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/telegram/webhook": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 텔레그램 웹훅 수신
         * @description 텔레그램 서버에서 전송하는 Update(버튼 클릭, 메시지 등)를 수신하여 처리.
         */
        post: operations["handleWebhook"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/trading-cycles/{id}/execute": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** 매매 수동 실행 (INFINITE 전용) */
        post: operations["executeManually"];
        /** 수동 실행 주문 취소 (오늘 PLACED 주문 전체) */
        delete: operations["cancelExecute"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/internal/fida-orders": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * FIDA 주문 실행
         * @description FIDA 계좌로 즉시 지정가 매매 주문 접수. X-Internal-Token 헤더 필수.
         */
        post: operations["placeFidaOrder"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/fcm/tokens": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * FCM 토큰 등록
         * @description body: {"token": "...", "platform": "WEB"}
         */
        post: operations["registerToken"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/auth/kakao/callback": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 카카오 로그인
         * @description 카카오 OAuth 인가 코드로 로그인. 신규 사용자는 PENDING 상태로 가입 후 관리자 승인 대기.
         */
        post: operations["kakaoCallback"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/auth/dev-token": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * [DEV] 개발용 JWT 토큰 발급 — 로컬 프로파일 전용
         * @description 고정 UUID(00000000-…-0001) 테스트 유저를 자동 생성·승인 후 JWT 반환. 매번 호출해도 idempotent.
         */
        post: operations["devToken"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/auth/dev-approve/{userId}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * [DEV] UID로 사용자 승인 — 로컬 프로파일 전용
         * @description 지정한 userId를 APPROVED 상태로 변경. Telegram 없이 로컬 승인 처리 시 사용.
         */
        post: operations["devApprove"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/auth/dev-admin-token": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /** 로컬 전용 ADMIN 테스트 토큰 발급 */
        post: operations["devAdminToken"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/auth/approval-requests": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * 승인 재신청
         * @description 거절(24시간) 또는 대기(1시간) 쿨다운 이후 재신청 가능.
         */
        post: operations["requestApproval"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 내 계좌 목록 조회
         * @description 로그인한 사용자의 전체 계좌 목록 반환. 계좌번호는 마지막 4자리만 노출.
         */
        get: operations["list"];
        put?: never;
        /**
         * 계좌 등록
         * @description KIS/Toss 계좌 및 자격증명을 AES-256 암호화하여 저장.
         */
        post: operations["register"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/trading-cycles": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 거래 사이클 목록 조회 */
        get: operations["list_1"];
        put?: never;
        /** 거래 사이클 등록 */
        post: operations["register_1"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/connection-tests": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        /**
         * KIS API 연결 테스트
         * @description appKey/appSecret으로 KIS OAuth 토큰 발급을 시도해 자격증명을 검증합니다.
         */
        post: operations["testConnection"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/trading-cycles/{id}/resume": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        /** 거래 사이클 재개 */
        patch: operations["resume"];
        trace?: never;
    };
    "/api/trading-cycles/{id}/pause": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        /** 거래 사이클 중지 */
        patch: operations["pause"];
        trace?: never;
    };
    "/api/settings/notification-channel": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        /**
         * 알림 채널 변경
         * @description TELEGRAM / FCM / ALL 중 선택. body: {"channel": "FCM"}
         */
        patch: operations["updateNotificationChannel"];
        trace?: never;
    };
    "/api/settings/balance-check": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        /**
         * 잔고 검증 설정
         * @description false 시 실잔고 미확인 모드 — 예수금 부족해도 전략 등록·재등록 가능. body: {"enabled": false}
         */
        patch: operations["updateBalanceCheck"];
        trace?: never;
    };
    "/api/admin/users/{userId}/status": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        /**
         * 사용자 상태 변경
         * @description status: ACTIVE(승인), REJECTED(거절)
         */
        patch: operations["updateStatus"];
        trace?: never;
    };
    "/api/admin/users/{userId}/role": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        /** 역할 변경 */
        patch: operations["changeRole"];
        trace?: never;
    };
    "/api/trading-cycles": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 내 전체 거래 사이클 목록 */
        get: operations["listMine"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/trading-cycles/{strategyId}/history": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 전략 거래 이력 조회 */
        get: operations["getStrategyHistory"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/trading-cycles/{id}/preview": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 전략 다음 주문 미리보기 */
        get: operations["preview"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/trades": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 거래 내역 조회
         * @description 날짜 범위와 종목으로 필터링. 기본: 최근 30일, 종목 SOXL.
         */
        get: operations["getTrades"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/trades/stream": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get: operations["stream"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/privacy-trades/base/latest": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 현재 사이클 기준가 조회
         * @description 오늘 이후 거래일 중 가장 미래의 privacy_trades_master 행에서 current_cycle_start를 반환. 전략 등록 시 매수 배수 MAX 계산에 사용.
         */
        get: operations["getLatestBase"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/portfolio/snapshots": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 포트폴리오 스냅샷 목록
         * @description 지정 기간의 포트폴리오 스냅샷 목록 반환. 기본: 최근 30일.
         */
        get: operations["getPortfolioSnapshots"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/portfolio/current": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 현재 포트폴리오 조회
         * @description 가장 최근 포트폴리오 스냅샷 1건 반환.
         */
        get: operations["getPortfolioCurrent"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/meta": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 전체 메타 번들 조회 */
        get: operations["getBundle"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/meta/tickers": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 티커 목록 */
        get: operations["getTickers"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/meta/strategy-types": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 전략 타입 목록 */
        get: operations["getStrategyTypes"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/meta/strategy-statuses": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 전략 상태 목록 */
        get: operations["getStrategyStatuses"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/meta/cycle-seed-types": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 연속 사이클 정책 목록 */
        get: operations["getCycleSeedTypes"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/meta/brokers": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 브로커 목록 */
        get: operations["getBrokers"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/market/session": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 현재 시장 세션 조회
         * @description 현재 KST 기준 미국 시장 세션을 반환합니다.
         *     DIRECT: 프리마켓+정규장 (주문 가능) — DST: 17:00~05:00 / 비DST: 18:00~06:00 KST
         *     BLOCKED: 장마감~프리마켓 전 (주문 불가) — DST: 05:00~17:00 / 비DST: 06:00~18:00 KST
         */
        get: operations["getSession"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/market/holidays": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 월별 미국 시장 휴장일 조회 */
        get: operations["getHolidays"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/auth/status-stream": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 승인 상태 SSE 스트림
         * @description PENDING 상태 사용자가 연결. 관리자 승인/거절 시 이벤트 수신 후 브라우저 자동 이동.
         */
        get: operations["statusStream"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/auth/me": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 내 정보 조회
         * @description 현재 로그인한 사용자의 프로필 및 계정 상태 반환.
         */
        get: operations["me"];
        put?: never;
        post?: never;
        /**
         * 회원 탈퇴
         * @description 본인 계정 및 모든 연관 데이터(계좌, 거래내역 등)를 즉시 삭제합니다.
         */
        delete: operations["deleteMe"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/users": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 사용자 목록 조회 */
        get: operations["listUsers"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/trades": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get: operations["listTrades"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/privacy-trade-bases": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get: operations["listBases"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/dashboard/stats": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /** 대시보드 통계 조회 */
        get: operations["getStats"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/audit-logs": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get: operations["listAuditLogs"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/anomalies": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get: operations["getAnomalies"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/accounts": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get: operations["listAccounts"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/_ping": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get: operations["ping"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/trades": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 체결 내역 조회
         * @description KIS API TTTS3035R — 지정 기간 동안의 체결된 주문 목록 조회.
         */
        get: operations["getTrades_1"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/profit": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 기간손익 조회
         * @description KIS API TTTS3039R — 지정 기간 동안의 종목별 실현손익 및 수익률 조회.
         */
        get: operations["getPeriodProfit"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/prices": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 복수 종목 현재가 조회
         * @description KIS API HHDFS76410000 — 최대 10개 종목의 현재가를 단일 호출로 조회. tickers는 TQQQ/SOXL/USD 중 1~10개.
         */
        get: operations["getPrices"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/portfolio": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 현재 잔고 조회
         * @description KIS API CTRP6504R — 체결 기준 현재 보유 종목별 잔고 및 평가손익 조회.
         */
        get: operations["getPresentBalance"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/margin": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 해외증거금 조회
         * @description KIS API TTTC2101R — USD·KRW 통화별 통합주문가능금액(예수금) 조회.
         */
        get: operations["getMargin"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/daily-trades": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 일별 거래내역 조회
         * @description KIS API CTOS4001R — 지정 기간 동안의 일별 거래내역 조회.
         */
        get: operations["getDailyTransactions"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/accounts/{accountId}/cycle-history": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        /**
         * 사이클 이력 조회
         * @description DB의 trading_cycle_history 테이블 기반 계좌 거래 이력 조회. KIS API 미호출.
         */
        get: operations["getCycleHistory"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/orders/{orderId}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        /** 개별 주문 취소 */
        delete: operations["cancelOrder"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/fcm/tokens/{token}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        /**
         * FCM 토큰 삭제
         * @description 로그아웃 또는 알림 비활성화 시 호출
         */
        delete: operations["unregisterToken"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/api/admin/users/{userId}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        get?: never;
        put?: never;
        post?: never;
        /** 사용자 삭제 */
        delete: operations["deleteUser"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
}
export type webhooks = Record<string, never>;
export interface components {
    schemas: {
        TradingCycleRequest: {
            /**
             * @description 전략 종류 (등록 시 필수)
             * @example INFINITE
             * @enum {string}
             */
            type: "INFINITE" | "PRIVACY";
            /**
             * @description 거래 종목 (PRIVACY는 SOXL 자동 고정, INFINITE 기본=TQQQ)
             * @example TQQQ
             * @enum {string}
             */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /**
             * @description 초기 입금액 (PRIVACY: 배수 자동 산출 기준)
             * @example 2000
             */
            initialUsdDeposit?: number;
            /**
             * @description 연속 사이클 정책 (null이면 NONE)
             * @example NONE
             * @enum {string}
             */
            cycleSeedType?: "NONE" | "MAINTAIN" | "MAX";
            /**
             * Format: int32
             * @description 분할 수 (20/30/40, null이면 20)
             * @example 20
             */
            divisionCount?: number;
        };
        TradingCycleResponse: {
            /**
             * Format: uuid
             * @description 거래 사이클 고유 ID
             */
            id?: string;
            /**
             * Format: uuid
             * @description 소속 계좌 ID
             */
            accountId?: string;
            /**
             * @description 전략 종류
             * @example INFINITE
             */
            type?: string;
            /**
             * @description 사이클 상태
             * @example ACTIVE
             */
            status?: string;
            /**
             * @description 거래 종목
             * @example TQQQ
             */
            ticker?: string;
            /**
             * @description 초기 입금액
             * @example 2000
             */
            initialUsdDeposit?: number;
            /**
             * @description 연속 사이클 정책
             * @example NONE
             */
            cycleSeedType?: string;
            /**
             * Format: int32
             * @description 분할 수 (20/30/40)
             * @example 20
             */
            divisionCount?: number;
            /**
             * @description 리버스모드 활성 여부 (소진 후 모드)
             * @example false
             */
            isReverseMode?: boolean;
        };
        TelegramUpdateRequest: {
            botToken: string;
            chatId: string;
        };
        AccountRequest: {
            /**
             * @description 계좌 별명
             * @example 내 메인 계좌
             */
            nickname: string;
            /**
             * @description 계좌번호 — KIS: XXXXXXXX-XX (예: 74420614-01), Toss: XXX-XX-XXXXXX (예: 131-01-001931)
             * @example 74420614-01
             */
            accountNo: string;
            /**
             * @description API 앱 키 (KIS App Key / Toss Client ID)
             * @example PSxxxxxxxxxx
             */
            appKey?: string;
            /** @description API 앱 시크릿 (KIS App Secret / Toss Client Secret) */
            secretKey?: string;
            /**
             * @description 증권사 — null이면 KIS 기본값 적용
             * @example KIS
             * @enum {string}
             */
            broker?: "KIS" | "TOSS";
        };
        AccountResponse: {
            /**
             * Format: uuid
             * @description 계좌 고유 ID
             */
            id?: string;
            /**
             * @description 계좌 별명
             * @example 내 메인 계좌
             */
            nickname?: string;
            /**
             * @description 마스킹된 계좌번호
             * @example ****4614
             */
            accountNoMasked?: string;
            /**
             * @description 증권사
             * @example KIS
             */
            broker?: string;
        };
        CallbackQuery: {
            id?: string;
            data?: string;
            message?: components["schemas"]["Message"];
        };
        Chat: {
            /** Format: int64 */
            id?: number;
        };
        Message: {
            /** Format: int64 */
            message_id?: number;
            chat?: components["schemas"]["Chat"];
            text?: string;
        };
        TelegramUpdate: {
            /** Format: int64 */
            update_id?: number;
            message?: components["schemas"]["Message"];
            callback_query?: components["schemas"]["CallbackQuery"];
        };
        ExecuteOrdersResponse: {
            orders?: components["schemas"]["PlacedOrderItem"][];
        };
        PlacedOrderItem: {
            /** Format: uuid */
            id?: string;
            ticker?: string;
            direction?: string;
            orderType?: string;
            /** Format: int32 */
            quantity?: number;
            price?: number;
        };
        FidaOrderCommand: {
            /** Format: date */
            tradeDate: string;
            /** @enum {string} */
            ticker: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            currentCycleStart: number;
            currentCycleRealizedPnl: number;
            avgPrice?: number;
            /** Format: int32 */
            holdings?: number;
            orders?: components["schemas"]["Order"][];
            buyQuantityValid?: boolean;
        };
        Order: {
            /** Format: uuid */
            id?: string;
            /** Format: uuid */
            accountId?: string;
            /** Format: uuid */
            strategyCycleId?: string;
            /** Format: date */
            tradeDate?: string;
            /** @enum {string} */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /** @enum {string} */
            orderType?: "LOC" | "MOC" | "LIMIT";
            /** @enum {string} */
            direction?: "BUY" | "SELL";
            /** Format: int32 */
            quantity?: number;
            price?: number;
            /** @enum {string} */
            status?: "PLANNED" | "PLACED" | "FILLED" | "PARTIALLY_FILLED" | "FAILED" | "CANCELLED";
            externalOrderId?: string;
            /** Format: int32 */
            filledQuantity?: number;
            filledPrice?: number;
        };
        FidaOrderResponse: {
            /** Format: uuid */
            id?: string;
            /** Format: date */
            tradeDate?: string;
            /** @enum {string} */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            currentCycleStart?: number;
            currentCycleRealizedPnl?: number;
            avgPrice?: number;
            /** Format: int32 */
            holdings?: number;
            orders?: components["schemas"]["Order"][];
        };
        FcmTokenRequest: {
            token: string;
            platform: string;
        };
        KakaoCallbackRequest: {
            /**
             * @description 카카오 OAuth 인가 코드
             * @example xxxxxxxxxxxxxxxxxxxxxxxx
             */
            code?: string;
            /**
             * @description 카카오 리다이렉트 URI
             * @example https://example.com/oauth/kakao
             */
            redirectUri?: string;
        };
        KakaoLoginResponse: {
            /**
             * @description 발급된 JWT 액세스 토큰
             * @example eyJhbGciOiJFUzI1NiJ9.eyJzdWIiOiIwMDAwMDAwMC0wMDAwLTAwMDAtMDAwMC0wMDAwMDAwMDAwMDEifQ.sig
             */
            accessToken?: string;
            /**
             * @description 토큰 타입
             * @example bearer
             */
            tokenType?: string;
            /**
             * Format: int64
             * @description 토큰 유효 기간 (초)
             * @example 604800
             */
            expiresIn?: number;
            /** @description 로그인한 사용자 정보 */
            user?: components["schemas"]["UserResponse"];
        };
        UserResponse: {
            /**
             * Format: uuid
             * @description 사용자 고유 ID
             * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
             */
            id?: string;
            /**
             * @description 닉네임
             * @example 홍길동
             */
            nickname?: string;
            /**
             * @description 계정 상태 (PENDING/APPROVED/REJECTED)
             * @example APPROVED
             * @enum {string}
             */
            status?: "PENDING" | "ACTIVE" | "REJECTED";
            /**
             * @description 텔레그램 알림 설정 여부
             * @example true
             */
            hasTelegram?: boolean;
            /**
             * @description 역할 (USER/ADMIN)
             * @example USER
             * @enum {string}
             */
            role?: "USER" | "ADMIN";
            /**
             * @description 텔레그램 봇 username (null이면 미연결)
             * @example narafu_kista_bot
             */
            telegramBotUsername?: string;
            /**
             * @description 알림 채널 (TELEGRAM/FCM/ALL)
             * @example TELEGRAM
             * @enum {string}
             */
            notificationChannel?: "NONE" | "TELEGRAM" | "FCM" | "ALL";
            /**
             * @description 전략 등록·재등록 시 실잔고 검증 여부 (false=바이패스)
             * @example true
             */
            balanceCheckEnabled?: boolean;
        };
        TokenResponse: {
            /**
             * @description JWT 액세스 토큰
             * @example eyJhbGciOiJFUzI1NiJ9.eyJzdWIiOiIwMDAwMDAwMC0wMDAwLTAwMDAtMDAwMC0wMDAwMDAwMDAwMDEifQ.sig
             */
            accessToken?: string;
            /**
             * @description 토큰 타입
             * @example bearer
             */
            tokenType?: string;
            /**
             * Format: int64
             * @description 토큰 유효 기간 (초)
             * @example 604800
             */
            expiresIn?: number;
        };
        TestConnectionRequest: {
            appKey?: string;
            appSecret?: string;
            /** Format: uuid */
            accountId?: string;
        };
        NotificationChannelRequest: {
            channel: string;
        };
        BalanceCheckRequest: {
            enabled?: boolean;
        };
        StatusRequest: {
            /** @enum {string} */
            status?: "PENDING" | "ACTIVE" | "REJECTED";
        };
        RoleRequest: {
            /** @enum {string} */
            role?: "USER" | "ADMIN";
        };
        CycleHistoryPageResponse: {
            items?: components["schemas"]["CycleHistoryResponse"][];
            nextCursor?: string;
            hasMore?: boolean;
        };
        CycleHistoryResponse: {
            createdAt?: string;
            ticker?: string;
            /** Format: int32 */
            holdings?: number;
            closingPrice?: number;
            avgPrice?: number;
            usdDeposit?: number;
        };
        NextOrdersResponse: {
            /** Format: date */
            tradeDate?: string;
            position?: components["schemas"]["PositionSnapshot"];
            orders?: components["schemas"]["OrderItem"][];
            /** @enum {string} */
            skipReason?: "NO_CYCLE_HISTORY" | "INSUFFICIENT_BALANCE" | "NO_PRIVACY_BASE";
        };
        OrderItem: {
            /** @enum {string} */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /** @enum {string} */
            orderType?: "LOC" | "MOC" | "LIMIT";
            /** @enum {string} */
            direction?: "BUY" | "SELL";
            /** Format: int32 */
            quantity?: number;
            price?: number;
        };
        PositionSnapshot: {
            /** @enum {string} */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /** Format: int32 */
            holdings?: number;
            averagePrice?: number;
            usdDeposit?: number;
            priceOffsetRate?: number;
            /** Format: double */
            currentRound?: number;
            unitAmount?: number;
            referencePrice?: number;
            targetPrice?: number;
            totalAssets?: number;
        };
        TradeHistoryResponse: {
            /**
             * Format: uuid
             * @description 거래 내역 고유 ID
             * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
             */
            id?: string;
            /**
             * Format: date
             * @description 거래 날짜
             * @example 2025-01-15
             */
            tradeDate?: string;
            /**
             * @description 거래 종목
             * @example SOXL
             * @enum {string}
             */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /**
             * @description 주문 유형 (LOC/MOC/LIMIT)
             * @example LOC
             * @enum {string}
             */
            orderType?: "LOC" | "MOC" | "LIMIT";
            /**
             * @description 매매 방향 (BUY/SELL)
             * @example BUY
             * @enum {string}
             */
            direction?: "BUY" | "SELL";
            /**
             * Format: int32
             * @description 주문 수량
             * @example 5
             */
            quantity?: number;
            /**
             * @description 주문 단가 (USD)
             * @example 85.5
             */
            price?: number;
            /**
             * @description 주문 상태 (PLACED/FAILED)
             * @example PLACED
             * @enum {string}
             */
            status?: "PLANNED" | "PLACED" | "FILLED" | "PARTIALLY_FILLED" | "FAILED" | "CANCELLED";
            /**
             * @description 증권사 주문번호
             * @example 0000123456
             */
            orderId?: string;
        };
        SseEmitter: {
            /** Format: int64 */
            timeout?: number;
        };
        TelegramSettingsResponse: {
            /**
             * @description 텔레그램 설정 여부
             * @example true
             */
            configured?: boolean;
            /**
             * @description 텔레그램 채팅 ID (botToken은 보안상 미노출)
             * @example -1001234567890
             */
            chatId?: string;
        };
        PrivacyCurrentBaseResponse: {
            /**
             * @description 대상 종목 코드
             * @example SOXL
             */
            ticker?: string;
            /**
             * @description 현재 사이클 시작 기준가 ($)
             * @example 25.43
             */
            currentCycleStart?: number;
            /**
             * Format: date
             * @description 기준 매매표 거래일
             * @example 2026-05-26
             */
            tradeDate?: string;
        };
        PortfolioSnapshotResponse: {
            /**
             * Format: uuid
             * @description 이력 고유 ID
             */
            id?: string;
            /**
             * @description 거래 종목
             * @example SOXL
             * @enum {string}
             */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /**
             * Format: int32
             * @description 보유 수량
             * @example 30
             */
            holdings?: number;
            /**
             * @description 종가 (USD, null이면 PRIVACY 또는 초기 등록)
             * @example 26
             */
            closingPrice?: number;
            /**
             * @description 평균매입단가 (USD, 보유수량 0이면 null)
             * @example 25
             */
            avgPrice?: number;
            /**
             * @description 평가금액 (USD) = closingPrice × holdings (closingPrice null이면 0)
             * @example 780
             */
            marketValueUsd?: number;
            /**
             * @description 예수금 (통합주문가능금액, USD)
             * @example 500
             */
            usdDeposit?: number;
            /**
             * @description 총 자산 (평가금액 + 예수금, USD)
             * @example 1280
             */
            totalAssetUsd?: number;
            /**
             * Format: date-time
             * @description 기록 일시 (UTC)
             * @example 2025-01-15T07:00:00Z
             */
            createdAt?: string;
        };
        EnumMeta: {
            code?: string;
            label?: string;
            description?: string;
        };
        MetaBundle: {
            strategyTypes?: components["schemas"]["StrategyTypeMeta"][];
            tickers?: components["schemas"]["TickerMeta"][];
            brokers?: components["schemas"]["EnumMeta"][];
            strategyStatuses?: components["schemas"]["EnumMeta"][];
            cycleSeedTypes?: components["schemas"]["EnumMeta"][];
        };
        StrategyTypeMeta: {
            code?: string;
            description?: string;
            availableTickers?: string[];
        };
        TickerMeta: {
            code?: string;
            description?: string;
            targetProfitRate?: number;
        };
        MarketSessionResponse: {
            /** @enum {string} */
            session?: "DIRECT" | "BLOCKED";
            isDst?: boolean;
        };
        AdminUserResponse: {
            /**
             * Format: uuid
             * @description 사용자 고유 ID
             */
            id?: string;
            /** @description 닉네임 */
            nickname?: string;
            /**
             * @description 계정 상태
             * @enum {string}
             */
            status?: "PENDING" | "ACTIVE" | "REJECTED";
            /**
             * @description 역할
             * @enum {string}
             */
            role?: "USER" | "ADMIN";
            /**
             * Format: date-time
             * @description 가입 일시
             */
            createdAt?: string;
        };
        AdminTradeResponse: {
            /** Format: uuid */
            id?: string;
            /** Format: uuid */
            userId?: string;
            ownerNickname?: string;
            /** Format: date */
            tradeDate?: string;
            ticker?: string;
            direction?: string;
            orderType?: string;
            /** Format: int32 */
            quantity?: number;
            price?: number;
            status?: string;
        };
        AdminPrivacyBaseResponse: {
            /** Format: uuid */
            id?: string;
            /** Format: date */
            tradeDate?: string;
            ticker?: string;
            currentCycleStart?: number;
            currentCycleRealizedPnl?: number;
            avgPrice?: number;
            /** Format: int32 */
            holdings?: number;
            orders?: components["schemas"]["OrderLine"][];
        };
        OrderLine: {
            /** Format: uuid */
            id?: string;
            direction?: string;
            orderType?: string;
            price?: number;
            /** Format: int32 */
            quantity?: number;
        };
        AdminDashboardResponse: {
            /**
             * Format: int64
             * @description 전체 사용자 수
             */
            totalUsers?: number;
            /**
             * Format: int64
             * @description 승인 대기 수
             */
            pendingCount?: number;
            /**
             * Format: int64
             * @description 승인된 수
             */
            activeCount?: number;
            /**
             * Format: int64
             * @description 거절된 수
             */
            rejectedCount?: number;
            /**
             * Format: int64
             * @description 전체 계좌 수
             */
            totalAccounts?: number;
        };
        AdminAuditLogResponse: {
            /** Format: uuid */
            id?: string;
            /** Format: uuid */
            adminId?: string;
            action?: string;
            targetType?: string;
            /** Format: uuid */
            targetId?: string;
            payload?: {
                [key: string]: Record<string, never>;
            };
            /** Format: date-time */
            createdAt?: string;
        };
        AccountItem: {
            /** Format: uuid */
            id?: string;
            /** Format: uuid */
            userId?: string;
            ownerNickname?: string;
            accountNoMasked?: string;
            broker?: string;
        };
        AdminAnomaliesResponse: {
            pausedAccounts?: components["schemas"]["AccountItem"][];
            inactiveAccounts?: components["schemas"]["AccountItem"][];
        };
        AdminAccountResponse: {
            /** Format: uuid */
            id?: string;
            /** Format: uuid */
            userId?: string;
            ownerNickname?: string;
            accountNoMasked?: string;
            broker?: string;
        };
        PingResponse: {
            status?: string;
            adminId?: string;
        };
        ExecutionResponse: {
            /** Format: date */
            tradeDate?: string;
            /** @enum {string} */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /** @enum {string} */
            direction?: "BUY" | "SELL";
            /** Format: int32 */
            quantity?: number;
            price?: number;
            amountUsd?: number;
            orderId?: string;
        };
        ItemDto: {
            tradeDate?: string;
            /** @enum {string} */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /** Format: int32 */
            quantity?: number;
            avgBuyPrice?: number;
            avgSellPrice?: number;
            realizedProfit?: number;
            returnRate?: number;
            exchangeCode?: string;
        };
        PeriodProfitResponse: {
            items?: components["schemas"]["ItemDto"][];
            totalRealizedProfit?: number;
            totalReturnRate?: number;
        };
        /** @description 복수 종목 현재가 조회 응답 */
        MultiPriceResponse: {
            /** @description 종목별 현재가 목록 */
            prices?: components["schemas"]["TickerPrice"][];
        };
        TickerPrice: {
            /**
             * @description 종목 코드
             * @example TQQQ
             * @enum {string}
             */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /**
             * @description 현재가 (USD)
             * @example 120.5
             */
            price?: number;
        };
        PortfolioSummaryResponse: {
            positions?: components["schemas"]["PositionDto"][];
            summary?: components["schemas"]["SummaryDto"];
        };
        PositionDto: {
            /** @enum {string} */
            ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            /** Format: int32 */
            holdings?: number;
            avgPrice?: number;
            currentPrice?: number;
            evalAmountUsd?: number;
            profitLossUsd?: number;
            profitRate?: number;
            exchangeCode?: string;
        };
        SummaryDto: {
            totalAssetUsd?: number;
            totalEvalProfit?: number;
            totalReturnRate?: number;
            totalAssetUsdActual?: number;
            evalProfitUsdSum?: number;
            usdDeposit?: number;
            posEvalUsd?: number;
            exchangeRateKrwPerUsd?: number;
        };
        MarginResponse: {
            /** @enum {string} */
            currency?: "KRW" | "USD" | "JPY";
            integratedOrderableAmount?: number;
            foreignOrderableAmount?: number;
            purchasableAmount?: number;
            usdToKrwRate?: number;
        };
        DailyTransactionResponse: {
            items?: components["schemas"]["ItemDto"][];
            summary?: components["schemas"]["SummaryDto"];
        };
        CancelOrdersResponse: {
            /** Format: int32 */
            cancelledCount?: number;
            /** Format: int32 */
            failedCount?: number;
        };
    };
    responses: never;
    parameters: never;
    requestBodies: never;
    headers: never;
    pathItems: never;
}
export type $defs = Record<string, never>;
export interface operations {
    update: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                id: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["TradingCycleRequest"];
            };
        };
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TradingCycleResponse"];
                };
            };
        };
    };
    delete: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description No Content */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    getTelegram: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TelegramSettingsResponse"];
                };
            };
        };
    };
    updateTelegram: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["TelegramUpdateRequest"];
            };
        };
        responses: {
            /** @description 저장 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
            /** @description 유효하지 않은 Bot Token */
            400: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    removeTelegram: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 해제 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    update_1: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                id: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["AccountRequest"];
            };
        };
        responses: {
            /** @description 수정 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AccountResponse"];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AccountResponse"];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AccountResponse"];
                };
            };
        };
    };
    delete_1: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 삭제 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    handleWebhook: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["TelegramUpdate"];
            };
        };
        responses: {
            /** @description 처리 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    executeManually: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["ExecuteOrdersResponse"];
                };
            };
        };
    };
    cancelExecute: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["CancelOrdersResponse"];
                };
            };
        };
    };
    placeFidaOrder: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["FidaOrderCommand"];
            };
        };
        responses: {
            /** @description 기존 동일 데이터 존재 — 멱등 처리 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["FidaOrderResponse"];
                };
            };
            /** @description 신규 저장 성공 */
            201: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["FidaOrderResponse"];
                };
            };
            /** @description 잘못된 요청 */
            400: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["FidaOrderResponse"];
                };
            };
            /** @description 내부 토큰 인증 실패 */
            401: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["FidaOrderResponse"];
                };
            };
            /** @description 같은 날짜/종목에 내용이 다른 데이터 존재 */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["FidaOrderResponse"];
                };
            };
        };
    };
    registerToken: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["FcmTokenRequest"];
            };
        };
        responses: {
            /** @description 등록 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    kakaoCallback: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["KakaoCallbackRequest"];
            };
        };
        responses: {
            /** @description 로그인 성공 (JWT 반환) */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["KakaoLoginResponse"];
                };
            };
        };
    };
    devToken: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 토큰 발급 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TokenResponse"];
                };
            };
        };
    };
    devApprove: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                /**
                 * @description 승인할 사용자 ID
                 * @example 00000000-0000-0000-0000-000000000001
                 */
                userId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 승인 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    devAdminToken: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TokenResponse"];
                };
            };
        };
    };
    requestApproval: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 재신청 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
            /** @description 재신청 불가 상태 */
            400: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
            /** @description 쿨다운 중 — 응답 body에 재신청 가능 시각(ISO-8601) 포함 */
            429: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    list: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AccountResponse"][];
                };
            };
        };
    };
    register: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["AccountRequest"];
            };
        };
        responses: {
            /** @description 등록 성공 */
            201: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AccountResponse"];
                };
            };
            /** @description 이미 등록된 계좌이거나 잘못된 요청 */
            400: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AccountResponse"];
                };
            };
            /** @description 계좌번호가 자격증명과 일치하지 않음 */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AccountResponse"];
                };
            };
        };
    };
    list_1: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TradingCycleResponse"][];
                };
            };
        };
    };
    register_1: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                accountId: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["TradingCycleRequest"];
            };
        };
        responses: {
            /** @description Created */
            201: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TradingCycleResponse"];
                };
            };
        };
    };
    testConnection: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["TestConnectionRequest"];
            };
        };
        responses: {
            /** @description 연결 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
            /** @description appKey 또는 appSecret이 유효하지 않음 */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    resume: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description No Content */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    pause: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description No Content */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    updateNotificationChannel: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["NotificationChannelRequest"];
            };
        };
        responses: {
            /** @description 변경 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    updateBalanceCheck: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["BalanceCheckRequest"];
            };
        };
        responses: {
            /** @description 변경 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    updateStatus: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                userId: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["StatusRequest"];
            };
        };
        responses: {
            /** @description No Content */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    changeRole: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                userId: string;
            };
            cookie?: never;
        };
        requestBody: {
            content: {
                "application/json": components["schemas"]["RoleRequest"];
            };
        };
        responses: {
            /** @description No Content */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    listMine: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TradingCycleResponse"][];
                };
            };
        };
    };
    getStrategyHistory: {
        parameters: {
            query?: {
                from?: string;
                to?: string;
                cursor?: string;
                size?: number;
            };
            header?: never;
            path: {
                strategyId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["CycleHistoryPageResponse"];
                };
            };
        };
    };
    preview: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["NextOrdersResponse"];
                };
            };
        };
    };
    getTrades: {
        parameters: {
            query?: {
                /**
                 * @description 조회 시작일 (기본: 오늘 - 30일)
                 * @example 2025-01-01
                 */
                from?: string;
                /**
                 * @description 조회 종료일 (기본: 오늘)
                 * @example 2025-01-31
                 */
                to?: string;
                /**
                 * @description 거래 종목
                 * @example SOXL
                 */
                ticker?: "TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ";
            };
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TradeHistoryResponse"][];
                };
            };
        };
    };
    stream: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "text/event-stream": components["schemas"]["SseEmitter"];
                };
            };
        };
    };
    getLatestBase: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PrivacyCurrentBaseResponse"];
                };
            };
            /** @description 오늘 이후 날짜의 기준 매매표 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PrivacyCurrentBaseResponse"];
                };
            };
        };
    };
    getPortfolioSnapshots: {
        parameters: {
            query?: {
                /**
                 * @description 조회 시작일 (기본: 오늘 - 30일)
                 * @example 2025-01-01
                 */
                from?: string;
                /**
                 * @description 조회 종료일 (기본: 오늘)
                 * @example 2025-01-31
                 */
                to?: string;
            };
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PortfolioSnapshotResponse"][];
                };
            };
        };
    };
    getPortfolioCurrent: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PortfolioSnapshotResponse"];
                };
            };
        };
    };
    getBundle: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MetaBundle"];
                };
            };
        };
    };
    getTickers: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["TickerMeta"][];
                };
            };
        };
    };
    getStrategyTypes: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["StrategyTypeMeta"][];
                };
            };
        };
    };
    getStrategyStatuses: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["EnumMeta"][];
                };
            };
        };
    };
    getCycleSeedTypes: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["EnumMeta"][];
                };
            };
        };
    };
    getBrokers: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["EnumMeta"][];
                };
            };
        };
    };
    getSession: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 세션 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MarketSessionResponse"];
                };
            };
        };
    };
    getHolidays: {
        parameters: {
            query: {
                /**
                 * @description 연도
                 * @example 2026
                 */
                year: number;
                /**
                 * @description 월 (1-12)
                 * @example 6
                 */
                month: number;
            };
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": string[];
                };
            };
        };
    };
    statusStream: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description SSE 스트림 연결 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "text/event-stream": components["schemas"]["SseEmitter"];
                };
            };
        };
    };
    me: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["UserResponse"];
                };
            };
        };
    };
    deleteMe: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 탈퇴 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    listUsers: {
        parameters: {
            query?: {
                status?: "PENDING" | "ACTIVE" | "REJECTED";
            };
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AdminUserResponse"][];
                };
            };
        };
    };
    listTrades: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AdminTradeResponse"][];
                };
            };
        };
    };
    listBases: {
        parameters: {
            query?: {
                range?: string;
            };
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AdminPrivacyBaseResponse"][];
                };
            };
        };
    };
    getStats: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AdminDashboardResponse"];
                };
            };
        };
    };
    listAuditLogs: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AdminAuditLogResponse"][];
                };
            };
        };
    };
    getAnomalies: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AdminAnomaliesResponse"];
                };
            };
        };
    };
    listAccounts: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["AdminAccountResponse"][];
                };
            };
        };
    };
    ping: {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description OK */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PingResponse"];
                };
            };
        };
    };
    getTrades_1: {
        parameters: {
            query: {
                /**
                 * @description 조회 시작일
                 * @example 2025-01-01
                 */
                from: string;
                /**
                 * @description 조회 종료일
                 * @example 2025-01-31
                 */
                to: string;
            };
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["ExecutionResponse"][];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["ExecutionResponse"][];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["ExecutionResponse"][];
                };
            };
            /** @description KIS API 호출 실패 */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["ExecutionResponse"][];
                };
            };
        };
    };
    getPeriodProfit: {
        parameters: {
            query: {
                /**
                 * @description 조회 시작일
                 * @example 2025-01-01
                 */
                from: string;
                /**
                 * @description 조회 종료일
                 * @example 2025-01-31
                 */
                to: string;
            };
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PeriodProfitResponse"];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PeriodProfitResponse"];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PeriodProfitResponse"];
                };
            };
            /** @description KIS API 호출 실패 */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PeriodProfitResponse"];
                };
            };
        };
    };
    getPrices: {
        parameters: {
            query: {
                /**
                 * @description 조회할 종목 코드 목록 (TQQQ/SOXL/USD)
                 * @example TQQQ,SOXL,USD
                 */
                tickers: ("TQQQ" | "SOXL" | "USD" | "MAGX" | "FNGU" | "BULZ")[];
            };
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MultiPriceResponse"];
                };
            };
            /** @description tickers 1~10개 필요 또는 유효하지 않은 종목 */
            400: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MultiPriceResponse"];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MultiPriceResponse"];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MultiPriceResponse"];
                };
            };
            /** @description KIS API 호출 실패 */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MultiPriceResponse"];
                };
            };
        };
    };
    getPresentBalance: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PortfolioSummaryResponse"];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PortfolioSummaryResponse"];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PortfolioSummaryResponse"];
                };
            };
            /** @description KIS API 호출 실패 */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["PortfolioSummaryResponse"];
                };
            };
        };
    };
    getMargin: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MarginResponse"][];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MarginResponse"][];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MarginResponse"][];
                };
            };
            /** @description KIS API 호출 실패 */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["MarginResponse"][];
                };
            };
        };
    };
    getDailyTransactions: {
        parameters: {
            query: {
                /**
                 * @description 조회 시작일
                 * @example 2025-01-01
                 */
                from: string;
                /**
                 * @description 조회 종료일
                 * @example 2025-01-31
                 */
                to: string;
            };
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["DailyTransactionResponse"];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["DailyTransactionResponse"];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["DailyTransactionResponse"];
                };
            };
            /** @description KIS API 호출 실패 */
            503: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["DailyTransactionResponse"];
                };
            };
        };
    };
    getCycleHistory: {
        parameters: {
            query?: {
                /** @description 조회 시작일 (생략 시 전체) */
                from?: string;
                /** @description 조회 종료일 (생략 시 전체) */
                to?: string;
                /** @description 커서 (이전 응답의 nextCursor) */
                cursor?: string;
                /** @description 페이지 크기 (기본 50, 최대 200) */
                size?: number;
            };
            header?: never;
            path: {
                /**
                 * @description 계좌 ID
                 * @example a1b2c3d4-e5f6-7890-abcd-ef1234567890
                 */
                accountId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 조회 성공 */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["CycleHistoryPageResponse"];
                };
            };
            /** @description 내 계좌가 아님 */
            403: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["CycleHistoryPageResponse"];
                };
            };
            /** @description 계좌를 찾을 수 없음 */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "*/*": components["schemas"]["CycleHistoryPageResponse"];
                };
            };
        };
    };
    cancelOrder: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                orderId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description No Content */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    unregisterToken: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                token: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description 삭제 성공 */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
    deleteUser: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                userId: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description No Content */
            204: {
                headers: {
                    [name: string]: unknown;
                };
                content?: never;
            };
        };
    };
}
