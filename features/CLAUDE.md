...
| 도메인 | 슬라이스 | 주요 컴포넌트 |
|---|---|---|
| `auth` | `kakao-login` | `KakaoLoginButton` |
| `auth` | `logout` | `LogoutButton` |
| `auth` | `reapply` | `ReapplyButton` (1시간/24시간 쿨다운 localStorage) |
...
- **`auth/reapply`**: `reapply_last_requested_at`(pending, 1시간) / `reapply_rejected_last_at`(rejected, 24시간) localStorage 쿨다운
