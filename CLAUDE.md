...
- **TradesTab**: `AccountDetailTabs.tsx` 내부 로컬 함수 (export 없음) — 재사용 필요 시 인라인 구현. **self-fetching 컴포넌트** (`accountId`만 받아 내부에서 `getAccountCycleHistory` 직접 호출). 전체/7일/30일/직접입력 4가지 RangeType 상태 관리. `page.tsx`에서 cycleHistory pre-fetch 없음
...
- **`getAccountCycleHistory` 파라미터**: `params: { from?, to? }`, `token?` — 둘 다 optional. token 없으면 clientFetch(Route Handler `/api/accounts/[[...path]]` 경유). `{}` 전달 = 전체 기간. `getAccountProfit`/`getPortfolioSnapshots`과 동일 패턴
