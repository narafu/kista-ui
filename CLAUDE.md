...
- **Next.js Image `size-[X]` 클래스 금지**: `<Image width={N} height={N} className="size-[Npx]" />`처럼 CSS로 크기를 재설정하면 Next.js 16 Image가 `[browser] has either width or height modified, but not the other` 경고를 발생시킴. `width`/`height` props가 이미 크기를 고정하므로 CSS `size-[X]`/`w-[X]`/`h-[X]` 클래스는 제거할 것
