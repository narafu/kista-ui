...
- **SSE 프록시 Route Handler `UND_ERR_BODY_TIMEOUT`**: Node.js 내장 `fetch`(undici)의 기본 `bodyTimeout`은 300초 — SSE처럼 이벤트가 드문 장기 연결에서 5분마다 `failed to pipe response / Body Timeout Error`로 500. 고정 패턴: `import { Agent } from 'undici'` + `const sseAgent = new Agent({ bodyTimeout: 0 })` + fetch 옵션에 `// @ts-ignore` + `dispatcher: sseAgent` 추가. `undici`는 `npm install undici` 필요(Next.js transitive dep 아님)
