...
- **Next.js Image + Tailwind preflight 경고**: Tailwind v4 preflight가 `img { max-width: 100%; height: auto }` 를 적용해 height만 CSS로 수정 → Next.js 16 Image `[browser] has either width or height modified, but not the other` 경고 유발. 근본 원인은 `size-[X]`/`w-[X]`/`h-[X]` 클래스가 아니라 Tailwind preflight의 비대칭 CSS. **수정**: `globals.css` 끝에 `img[data-nimg] { max-width: revert; height: revert; }` 규칙이 이미 적용돼 있음. Image 컴포넌트에 별도 size 클래스 불필요
