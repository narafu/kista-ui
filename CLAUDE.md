...
- **API 날짜 파라미터**: `getTrades`/`getAccountTrades`/`getAccountProfit`/`getAccountReservationOrders` 모두 `{ from, to }` (ISO date string) — `buildDateQuery` 키 `from`/`to`로 통일됨. `getAccountDailyTrades` 함수명 존재하지 않음. `getPortfolioSnapshots`는 `{ from?, to? }` (optional — 미전달 시 서버 기본값 최근 30일)
