...
- **INFINITE vs PRIVACY UI 판별**: `'INFINITE'`/`'PRIVACY'` 리터럴 직접 사용 금지 (MetaProvider SSOT 원칙). `typeMeta?.availableTickers?.length`로 간접 판별 — INFINITE=다종목(`Ticker` 전체), PRIVACY=1종목(SOXL). 패턴: `(typeMeta?.availableTickers?.length ?? 0) > 1`
- **kista-api DTO**: `UserResponse`는 `{ id, nickname, status, hasTelegram, role, telegramBotUsername }`, `AccountResponse`는 `{ id, nickname, accountNoMasked, broker }`, `TradingCycleResponse`는 `{ id, accountId, type, status, ticker, multiple(BigDecimal→JSON number) }` — `types/` 참고
- **TradingCycleResponse.multiple 타입 불일치**: 백엔드 BigDecimal → JSON number(예: 1.0) 직렬화. 프론트 `Strategy.multiple: string`과 불일치 → `lib/api/strategies.ts`의 `normalizeStrategy()`에서 `String(s.multiple)` 강제 변환. 신규 KIS 엔드포인트 BigDecimal 필드도 동일 패턴 적용
