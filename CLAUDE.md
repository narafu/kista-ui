...
- `components/strategies/` — 전략 컴포넌트 (StrategyCard, StrategyList, StrategyFormDialog). StrategyForm은 `hooks/useStrategyForm.ts`(상태/로직) + `sections/`(4개 UI 섹션)으로 분리됨
...
- **커스텀 반응형 그리드**: Tailwind에 없는 `gridTemplateColumns`(예: `1fr 1fr 1.4fr`) — `globals.css` 끝에 `.sm\:kpi-grid { display:grid; }` 형태로 추가, 인라인 `gridTemplateColumns` 제거 필수 (명시도 충돌). 기존 정의: `sm\:kpi-grid`(sm=3컬럼`1fr 1fr 1.4fr`, lg=4컬럼`1fr 1.2fr 1fr 1fr` — KPI 카드 수 변경 시 lg 브레이크포인트 규칙도 함께 수정), `sm\:portfolio-grid`, `md\:profit-grid`, `lg\:form-grid`, `lg\:settings-grid`, `.rejected-reason-card`(반려카드 배경), `.error-page-bg`(에러페이지 radial-gradient 배경) (`.account-detail-row1`은 제거됨 — `grid grid-cols-2 gap-6` Tailwind 클래스로 대체)
- **AccountDetailTabs 레이아웃 구조**: 데스크탑 3행 구성 — 1행: `계좌요약(AccountSummaryCard) | 거래내역(TradesTab, accountId 기준)`, 2행: `전략(StrategyList) | 거래내역(StrategyTradesTab, strategyId 기준)`, 3행: `NextOrderPreviewCard(전폭)`. 모바일 탭: 요약/계좌 거래/전략/다음 주문 4개. `AccountSummaryCard`에 `account.broker` 표시. `ProfitStatsCard`(수익/손실통계)는 계좌 상세에 없음 — 통계 페이지 전용
...
- **disabled 버튼 툴팁**: `title` 속성은 브라우저 기본 딜레이(1~3초) 발생 — wrapper `div`에 `group` + 툴팁 div에 `opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50` 패턴 사용. disabled 버튼은 JS hover 이벤트 차단되지만 부모 div의 CSS hover는 정상 동작
...
- **CSS `--status-error` 토큰**: 반려/거절 상태 색상 (라이트 `#C8443A`, 다크 `#F87171`). `--status-error-bg`/`--status-error-border`도 함께 정의. `.rejected-reason-card` 유틸 클래스 사용 가능. 하드코딩 `#C8443A` 금지
...
- **KIS live 계좌 API**: `getMargin(accountId)` → `lib/api/accounts.ts`, `/api/accounts/{id}/margin` 프록시, 응답 `MarginItem[]`(USD `purchasableAmount`=매수가능금액). `getPrices(accountId, ['TQQQ','SOXL','USD'])` → `Record<string,number>`, `/api/accounts/{id}/prices?tickers=...` 프록시 — 두 경로 모두 기존 `accounts/[[...path]]` catch-all이 처리, 별도 Route Handler 불필요
- **KIS TTTC2101R 외화증거금 필드 주의**: `frcr_ord_psbl_amt1`(외화주문가능금액)은 통합증거금 OFF 시 0이 됨 — 이름만 보고 "통합증거금 무관"이라 추측 금지. 항상 유효한 필드는 `frcr_gnrl_ord_psbl_amt`(외화일반주문가능금액). `KisMarginAdapter` `purchasableAmount = max(itgr_ord_psbl_amt, frcr_gnrl_ord_psbl_amt)` 공식으로 통합증거금 ON/OFF 모두 커버. KIS API 응답 필드 가정 검증 시 `mcp__kis-trade-mcp__overseas_stock({ api_type: "foreign_margin", params: { tr_cont: "", depth: 0, max_depth: 10 } })`으로 실계좌 원본 응답 확인 가능
...
- **Self-fetching Client Component 초기 mount fetch skip**: 서버 prop으로 초기 데이터 수신 시 `useState(propValue)` 초기화 + `const isInitialMount = useRef(true)` → useEffect 첫 실행에서 `if (isInitialMount.current) { isInitialMount.current = false; return }` 로 초기 fetch 방지, state 변경 시에만 `clientFetch` 호출 (`MarketHolidayCalendar` 패턴)
- **ProfitStatsCard 차트 공백**: `getPortfolioSnapshots()` = DB 저장 스냅샷 기반 (실시간 KIS 아님). kista-api가 주기적으로 스냅샷 저장 안 하면 항상 "데이터가 없습니다"
- **PortfolioSnapshot 타입 (V49/V50 이후)**: `snapshotDate` 필드 제거됨 — 날짜 관련 로직은 반드시 `createdAt` 사용. `currentPrice`는 `number | null` — 표시 시 null 가드(`currentPrice != null ? ... : '-'`) 필수. `GET /api/portfolio/current` + `GET /api/portfolio/snapshots` 모두 동일 변경 적용
...
- **TradesTab / StrategyTradesTab**: `AccountDetailTabs.tsx` 내부 로컬 함수 (export 없음). 두 컴포넌트 모두 공통 `CycleHistoryTable` 내부 함수로 렌더링 위임. `TradesTab`은 `accountId`로 `getAccountCycleHistory` 호출, `StrategyTradesTab`은 `strategyId`로 `getStrategyCycleHistory` 호출 (strategyId 없으면 "전략이 없습니다" 표시). 전체/7일/30일/직접입력 4가지 RangeType. `page.tsx`에서 pre-fetch 없음
- **API 날짜 파라미터**: `getTrades`/`getAccountTrades`/`getAccountProfit`/`getAccountReservationOrders` 모두 `{ from, to }` (ISO date string) — `buildDateQuery` 키 `from`/`to`로 통일됨. `getAccountDailyTrades` 함수명 존재하지 않음. `getPortfolioSnapshots`/`getAccountCycleHistory`는 `{ from?, to? }` (optional — `{}` 전달 시 전체 기간 조회, 서버 기본값 적용)
- **`getAccountCycleHistory` / `getStrategyCycleHistory` 파라미터**: 둘 다 `params: { from?, to? }`, `token?`. `getAccountCycleHistory` → `/api/accounts/{id}/cycle-history` (Route Handler `/api/accounts/[[...path]]`). `getStrategyCycleHistory` → `/api/trading-cycles/{id}/history` (Route Handler `/api/trading-cycles/[[...path]]`). `{}` 전달 = 전체 기간(서버 파라미터 required라 400 반환 주의 — 기본값 '30d' 사용 권장)
- **`getDailyTransactions(accountId, {from, to}, token?)`**: `GET /api/accounts/{id}/daily-trades` — KIS 일별 체결 내역 조회. 응답 `DailyTransactionResult { items: DailyTransaction[], summary: DailyTransactionSummary }`. `types/trade.ts`에 타입 정의됨
...
- **`GET /api/trades` 현재 필드 (TradeHistory)**: `id`, `tradeDate`, `ticker`, `orderType`, `direction`, `quantity`, `price`, `status`, `kisOrderId` — `strategy`/`amountUsd`/`createdAt` 제거됨. 금액 표시: `(price * quantity).toFixed(2)`, 날짜: `tradeDate`
- **`AdminAnomalies` 현재 필드**: `pausedAccounts`, `inactiveAccounts` — `failedTrades` 제거됨 (항상 빈 배열이었으므로 UI에서도 섹션 제거)
...
- **SSE 프록시 Route Handler `UND_ERR_BODY_TIMEOUT` / `UND_ERR_HEADERS_TIMEOUT`**: undici 기본 `bodyTimeout`(300s) + `headersTimeout`(300s) 모두 SSE 장기 연결에서 5분마다 500 유발. 고정 패턴: `import { Agent } from 'undici'` + `const sseAgent = new Agent({ bodyTimeout: 0, headersTimeout: 0 })` + fetch 옵션에 `// @ts-ignore` + `dispatcher: sseAgent` 추가. `undici`는 `npm install undici` 필요(Next.js transitive dep 아님)
...
- **next-themes hydration mismatch**: `useTheme()`의 `resolvedTheme`/`theme`는 SSR에서 `undefined` → Client Component에서 직접 사용 시 hydration warning. 패턴: `const [mounted, setMounted] = useState(false)` + `useEffect(() => setMounted(true), [])` → `const isDark = mounted && resolvedTheme === 'dark'`. `mounted`가 false면 기본값(light) 렌더 (`ThemeToggle.tsx`, `ThemeCards.tsx` 참고)
- **Next.js Image + Tailwind preflight 경고**: Tailwind v4 preflight `img { height: auto }` (@layer base) + 비정사각형 이미지(logo.png=256×139)를 정사각형(32×32 등)으로 표시 시 → `img.height`(CSS auto 계산값)≠`getAttribute('height')` → `[browser] has either width or height modified` 경고. **CSS 방식 한계**: async 스크립트(React hydration)가 render-blocking CSS보다 먼저 실행될 수 있어 `@layer utilities h-[Npx]`나 unlayered 속성 선택자 모두 race condition 발생. **실제 해결책**: 각 `<Image>` 컴포넌트의 `style` prop에 `height: N, width: N` 직접 명시 — 인라인 style은 HTML에 포함되어 CSS 로딩과 무관하게 즉시 적용됨. `globals.css`에는 `img[data-nimg]` 속성 선택자 규칙이 보조 수단으로 존재하나 단독으로는 충분하지 않음
...
- **`skipReason` + position 배너 패턴**: `NextOrderPreviewCard`의 `INSUFFICIENT_BALANCE` 처리 — `preview.position`이 있으면 `⚠️ 단위금액 $X • 현재가 $Y • 부족 $Z` 배너(`bg-warn-bg`/`text-warn`), 없으면 텍스트 폴백. 계산 변수(`insufficientUnitAmount/CurrentPrice/Shortfall`)는 JSX 외부에서 선언 (IIFE 금지 규칙)
- **StatisticsController 응답 형식 (주의: kista-ui 타입과 불일치)**: KIS live API를 직접 호출하는 엔드포인트는 도메인 모델/전용 DTO를 그대로 반환 → kista-ui 소비 시 반드시 응답 형식을 대조하고 normalizer 또는 정확한 타입 적용 필요.
  - `GET /api/accounts/{id}/portfolio` → `PortfolioSummaryResponse { positions[{ticker(Ticker enum),holdings(int),avgPrice,currentPrice,evalAmountUsd,profitLossUsd,profitRate,exchangeCode}], summary{totalAssetUsd,totalEvalProfit,totalReturnRate} }`. kista-ui: `normalizePortfolio(strategyTicker?)`가 활성 전략의 ticker 종목을 우선 선택(`positions.find(p => p.ticker === strategyTicker) ?? positions[0]`), 없으면 positions[0] 폴백 후 `PortfolioSnapshot`으로 변환 (`app/(main)/accounts/[id]/page.tsx`). BigDecimal → `lib/utils.ts`의 `toNum()` 헬퍼로 null/string 안전 변환 (`import { toNum } from '@/lib/utils'`). **통화 주의**: KIS `CTRP6504R` API에서 `positions[].evalAmountUsd`(=`frcr_evlu_amt2`)는 USD이지만, `summary.totalAssetUsd`(=`tot_asst_amt`)와 `summary.totalEvalProfit`(=`tot_evlu_pfls_amt`)는 **원화(KRW)** — 필드명에 Usd가 붙어있어도 KRW임. 대시보드에서 ₩ 표시 필요, 두 통화를 혼합 계산(KRW - USD) 금지.
  - `GET /api/accounts/{id}/trades` → `Execution[] { tradeDate,ticker,direction,quantity,price,kisOrderId }`. kista-ui: `types/trade.ts`의 `Execution` 타입 사용 (`TradeHistory` 아님 — `id`/`orderType`/`status` 필드 없음). `AccountDetailTabs` key는 `${kisOrderId}-${tradeDate}-${ticker}` 합성키, 날짜는 `tradeDate`.
...
- **`unstable_cache` RSC 에러 핸들링**: background revalidation 실패 시 `.catch()` 체인이 동작하지 않는 경우 있음 → `await getCachedX()` 호출은 반드시 `try { accounts = await getCachedAccounts(token) } catch {}` 패턴 사용 (`.catch(() => [])` 체인 금지)
...
다른 작업 중 코딩 가이드라인·CLAUDE.md 규칙에 위배되는 코드를 발견하면 작업을 중단하지 말고, **작업 완료 후 별도로 리팩토링을 제안**할 것.

- 제안 형식: 위반 파일·라인 + 위반 규칙 + 권장 수정 방향을 간략히 명시
- 즉시 수정하지 말 것 — 사용자 승인 후 진행
- 현재 작업 범위 밖의 코드 변경은 절대 포함 금지
