...
async function proxy(request: NextRequest, pathSegments: string[]) {
  ...
  const url = `${API_BASE_URL}/api/trading-cycles${subPath}${request.nextUrl.search}`
  ...
  if (!res.ok) {
    if (res.status >= 500) console.error(`[trading-cycles${subPath} ${request.method}] ${res.status}`, await res.text().catch(() => ''))
